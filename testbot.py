...
def handle_shodka(message):
    if message.from_user.id in get_admin_ids(bot, message.chat.id):
        if message.reply_to_message.text != "0":
            ...
            config.shodka_message.clear()
            bot.send_message(message.chat.id, "Информация о сходке аннулирована",
...
def handle_help(message):
    bot.send_message(message.chat.id,
                     'Версия ' + config.bot_version + '. Бан за мат выдается методом рулетки. Отвечаю лишь на сообщения со словом Бот или'
